...
/***************************/
uint64_t n = 200000;  // width and height of the matrix
/****************************/
...
int getArrayVal(uint64_t *mat, int row, int col){
	return mat[row + col*n];
	}

void setArrayVal(uint64_t *mat, int row, int col, int val){
	mat[row + col*n] = val;
	}
...
int main(){
	...
	start = std::clock();
	uint64_t g = n*n;
	uint64_t *Toeplitz = new uint64_t[40000000000];
	cout << "HELP" << endl;
